...
def main():
    ...
    with tab1:
        ...
        if st.button("🤖 AI 자동 리포트 생성"):
            ...
            - **상관관계 분석**: 수치형 변수들(df.select_dtypes(include=['number'])) 간 상관관계만 분석하고, 히트맵을 포함해. 문자열 데이터는 제외해.
            - **회귀분석**: 수치형 변수들(df.select_dtypes(include=['number'])) 간 회귀분석을 수행하고, 산점도 또는 회귀선을 포함해. 문자열 데이터는 제외하고, 적절한 독립 변수와 종속 변수를 선택해.
